...
void sort_insertion(sortable_move_t* move_list, int num_of_moves, int mv_index) {
...
// Returns the index of the max element starting from mv_index
int find_max(sortable_move_t* move_list, int num_of_moves, int mv_index) {
  sortable_move_t max_mv = move_list[mv_index];
  int max_index = mv_index;
  for (int i = mv_index; i < num_of_moves; i++) {
    if (move_list[i] > max_mv) {
      max_mv = move_list[i];
      max_index = i;
    }
  }
